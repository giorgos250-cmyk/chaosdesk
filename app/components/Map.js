...
const PLAGUE_STYLE = {
  ...
  layers: [
    { id: 'background', type: 'background', paint: { 'background-color': '#060d15' } },
    {
      id: 'base-tiles', type: 'raster', source: 'carto-dark',
      paint: {
        'raster-opacity': 1,
  ...
  return (
    <button
      ...
      style={{
        ...
        color: DIM, lineHeight: 1, padding: '0 4px',
      }}
    >✕</button>
  );
}
...
function MobileHeader({ label, onClose }) {
  return (
    <div style={{
      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      padding: '10px 16px', borderBottom: `1px solid ${BORDER}`, flexShrink: 0,
    }}>
      <span style={{ fontFamily: 'Orbitron, monospace', fontSize: 10, letterSpacing: '0.18em', color: DIM, fontWeight: 500 }}>
        {label}
      </span>
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: 'rgba(6,13,21,0.98)', border: `1px solid ${BORDER}`,
        maxWidth: 480, width: '100%', position: 'relative', padding: 24,
      }}>
        <button onClick={onClose} style={{
          position: 'absolute', top: 12, right: 14,
          background: 'none', border: 'none', cursor: 'pointer',
          fontFamily: 'Share Tech Mono, monospace', fontSize: 18, color: DIM,
        }}>✕</button>
        ...
        {item.source_url && (
          <button onClick={() => window.open(item.source_url, '_blank')} style={{
...
function TopBar({ news, datetime, isMobile }) {
  const ticker = news.map(n => cleanText(n.description || '')).filter(Boolean).join('   ·   ') || 'Αναμονή δεδομένων…';
  const h = isMobile ? 44 : 48;
  return (
    <div style={{
      position: 'absolute', top: 0, left: 0, right: 0, height: h, zIndex: 200,
      background: 'rgba(6,13,21,0.93)', borderBottom: `1px solid ${BORDER}`,
      display: 'flex', alignItems: 'center',
    }}>
      <div style={{
        flexShrink: 0, padding: `0 ${isMobile ? 12 : 18}px`, height: '100%',
        borderRight: `1px solid ${BORDER}`,
        display: 'flex', alignItems: 'center', gap: 8,
      }}>
        <span style={{ fontFamily: 'Orbitron, monospace', fontSize: isMobile ? 12 : 15, letterSpacing: '0.2em', color: RED, fontWeight: 700 }}>
          HANTAMAP
        </span>
        <span style={{
      ...
      <div style={{ flex: 1, overflow: 'hidden', height: '100%', display: 'flex', alignItems: 'center' }}>
        <span style={{
          ...
          fontFamily: 'Share Tech Mono, monospace', fontSize: isMobile ? 10 : 12,
...
function LeftPanel({ news, onSelect, isMobile, isOpen, onClose }) {
  ...
  const panelStyle = isMobile
    ? {
        ...
        width: 'min(88vw, 340px)', bottom: 0,
        zIndex: 400, background: 'rgba(6,13,21,0.98)',
  ...
  return (
    <div style={panelStyle}>
      {isMobile
        ? <MobileHeader label="ΑΝΑΦΟΡΕΣ LIVE" onClose={onClose} />
        : (
          <div style={{
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                <span style={{
                  padding: '2px 6px', background: b.bg, border: `1px solid ${b.border}`, borderRadius: 2,
                  fontFamily: 'Share Tech Mono, monospace', fontSize: 9, letterSpacing: '0.08em', color: b.color,
                }}>{item.source}</span>
              ...
              {hovered && (
                <div style={{ marginTop: 5, fontFamily: 'Share Tech Mono, monospace', fontSize: 9, color: bColor, opacity: 0.8 }}>
...
function GlobalPanel({ confirmed, suspected, news, isMobile, isOpen, onClose }) {
  if (isMobile && !isOpen) return null;
  ...
  const panelStyle = isMobile
    ? {
        ...
        width: 'min(88vw, 320px)', bottom: 0,
        zIndex: 400, background: 'rgba(6,13,21,0.98)',
        border: `1px solid ${BORDER}`, borderRight: 'none',
        overflowY: 'auto',
      }
    : {
        position: 'absolute', top: 48, right: 0, width: 190,
        background: 'rgba(6,13,21,0.92)', borderLeft: `1px solid ${BORDER}`, zIndex: 200,
      };

  return (
    <div style={panelStyle}>
      {isMobile
        ? <MobileHeader label="ΠΑΓΚΟΣΜΙΑ ΕΙΚΟΝΑ" onClose={onClose} />
        : (
          <div style={{ padding: '10px 14px', borderBottom: `1px solid ${BORDER}`, background: `${RED}10` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: RED, flexShrink: 0, animation: 'liveDot 1.4s ease-in-out infinite' }} />
      ...
      {stats.map(s => (
        <div key={s.label} style={{
          ...
          <span style={{ fontFamily: 'Orbitron, monospace', fontSize: isMobile ? 22 : 20, color: s.color, fontWeight: 700 }}>
            {s.value || '—'}
          </span>
        </div>
      ))}
...
function BottomHUD({ confirmed, suspected, news, globalStats, isMobile, isOpen, onClose }) {
  if (isMobile && !isOpen) return null;

  const totalCases  = confirmed.reduce((s, c) => s + (c.cases  || 0), 0);
  const totalDeaths = confirmed.reduce((s, c) => s + (c.deaths || 0), 0);
  const countries   = new Set([...confirmed, ...suspected].map(c => c.location?.country).filter(Boolean)).size;
  ...
  if (isMobile) {
    return (
      <div style={{
        ...
        zIndex: 400, background: 'rgba(6,13,21,0.98)',
        borderTop: `1px solid ${BORDER}`, maxHeight: '62vh', overflowY: 'auto',
      }}>
        <MobileHeader label="HUD — ΣΤΑΤΙΣΤΙΚΑ" onClose={onClose} />

        <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          {/* Counter + grid */}
          <div style={{ display: 'flex', gap: 20, alignItems: 'center', borderBottom: `1px solid ${BORDER}30`, paddingBottom: 14 }}>
            <div>
              <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 8, color: LABEL, letterSpacing: '0.2em', fontWeight: 500 }}>ΚΡΟΥΣΜΑΤΑ</div>
              <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 36, color: RED, fontWeight: 700, lineHeight: 1 }}>{totalCases || '—'}</div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, auto)', gap: '6px 20px' }}>
              {statItems.map(s => (
                <div key={s.label}>
                  <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 8, color: LABEL, letterSpacing: '0.12em', fontWeight: 500 }}>{s.label}</div>
                  <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 18, color: '#ffffff', fontWeight: 700 }}>{s.value || '—'}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* WHO alerts */}
          <div>
            <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 8, color: LABEL, letterSpacing: '0.15em', marginBottom: 8, fontWeight: 500 }}>WHO ΕΙΔΟΠΟΙΗΣΕΙΣ</div>
            {!alerts.length
              ? <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 10, color: DIM }}>Καμία ειδοποίηση</div>
              : alerts.slice(0, 3).map((a, i) => (
                <div key={i} style={{
                  fontFamily: 'Rajdhani, sans-serif', fontSize: 13, lineHeight: 1.4, fontWeight: 500,
                  color: i === 0 ? ORANGE : DIM, marginBottom: 4,
                }}>
                  {a.title}
                </div>
              ))}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 8, color: LABEL, letterSpacing: '0.2em', fontWeight: 500 }}>ΚΡΟΥΣΜΑΤΑ</div>
        <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 34, color: RED, fontWeight: 700, lineHeight: 1 }}>{totalCases || '—'}</div>
      </div>
      ...
      <div style={{ flex: 1, padding: '10px 18px', display: 'flex', alignItems: 'center' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, auto)', gap: '3px 22px' }}>
          {statItems.map(s => (
            <div key={s.label}>
              <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 8, color: LABEL, letterSpacing: '0.15em', fontWeight: 500 }}>{s.label}</div>
              <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 17, color: '#ffffff', fontWeight: 700 }}>{s.value || '—'}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 8, color: LABEL, letterSpacing: '0.15em', marginBottom: 3, fontWeight: 500 }}>WHO ΕΙΔΟΠΟΙΗΣΕΙΣ</div>
        {!alerts.length
          ? <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 10, color: DIM }}>Καμία ειδοποίηση</div>
          : alerts.slice(0, 3).map((a, i) => (
            <div key={i} style={{
              fontFamily: 'Rajdhani, sans-serif', fontSize: 11, lineHeight: 1.25, fontWeight: 500,
              color: i === 0 ? ORANGE : DIM, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
            }}>{a.title}</div>
          ))}
      </div>
    </div>
...
function MobileTab({ side, label, color, onClick }) {
  const isBottom = side === 'bottom';
  return (
    <button
      onClick={onClick}
      style={{
        position: 'absolute', zIndex: 300, cursor: 'pointer',
        background: 'rgba(6,13,21,0.92)', border: `1px solid ${BORDER}`,
        color, fontFamily: 'Share Tech Mono, monospace',
        fontSize: 9, letterSpacing: '0.12em', fontWeight: 500,
        ...(side === 'left'   && { left: 0, top: '50%', transform: 'translateY(-50%)', padding: '14px 7px', borderLeft: 'none', writingMode: 'vertical-rl' }),
        ...(side === 'right'  && { right: 0, top: '50%', transform: 'translateY(-50%)', padding: '14px 7px', borderRight: 'none', writingMode: 'vertical-rl' }),
        ...(side === 'bottom' && { bottom: 0, left: '50%', transform: 'translateX(-50%)', padding: '6px 20px', borderBottom: 'none' }),
      }}
    >
      {side === 'left'  ? `▶ ${label}` : side === 'right' ? `${label} ◀` : `▲ ${label}`}
    </button>
  );
}
...
export default function MapView() {
  const [data, setData] = useState({ confirmed: [], suspected: [], news: [], globalStats: {}, riskAreas: [] });
  const [mapLoaded, setMapLoaded]       = useState(false);
  const [datetime, setDatetime]         = useState('');
  const [selectedNews, setSelectedNews] = useState(null);
  const [openPanel, setOpenPanel]       = useState(null); // 'news'|'stats'|'hud'|null
  ...
  useEffect(() => {
    const tick = () => setDatetime(new Date().toISOString().replace('T', ' ').slice(0, 19) + ' UTC');
    tick();
  ...
  useEffect(() => {
    ...
    const caseFeatures = [
      ...confirmed.map(c => ({
        ...
        properties: { type: 'confirmed', weight: 1.0, region: c.location.region || c.location.country || '', cases: c.cases || 0, date_reported: c.date_reported || '', source: c.source || '' },
      })),
      ...suspected.map(c => ({
        type: 'Feature',
        geometry: { type: 'Point', coordinates: [c.location.lng, c.location.lat] },
        properties: { type: 'suspected', weight: 0.4, region: c.location.region || c.location.country || '', cases: c.cases || 0, date_reported: c.date_reported || '', source: c.source || '' },
      })),
    ...
    const riskGJ = {
      ...
      features: (riskAreas || []).map(r => ({
        ...
        properties: { name: r.name, risk_level: r.risk_level, fillOpacity: 0.03 + r.risk_level * 0.018 },
      })),
    };

    ['case-circles', 'infection-heat', 'risk-fill'].forEach(id => { try { if (map.getLayer(id)) map.removeLayer(id); } catch (_) {} });
    ['cases', 'risk-areas'].forEach(id => { try { if (map.getSource(id)) map.removeSource(id); } catch (_) {} });

    map.addSource('risk-areas', { type: 'geojson', data: riskGJ });
    map.addLayer({ id: 'risk-fill', type: 'fill', source: 'risk-areas', paint: { 'fill-color': RED, 'fill-opacity': ['get', 'fillOpacity'] } });

    map.addSource('cases', { type: 'geojson', data: { type: 'FeatureCollection', features: caseFeatures } });
    ...
    map.addLayer({
      ...
      paint: {
        ...
        'heatmap-color': ['interpolate', ['linear'], ['heatmap-density'],
          0, 'rgba(0,0,0,0)', 0.1, 'rgba(40,0,0,0.3)', 0.3, 'rgba(100,0,0,0.6)',
          0.6, 'rgba(180,10,10,0.8)', 0.85, 'rgba(230,20,0,0.9)', 1, 'rgba(255,80,0,1)',
        ],
    ...
    map.addLayer({
      id: 'case-circles', type: 'circle', source: 'cases',
      paint: {
    ...
    // Circle click
    if (clickHandlerRef.current) map.off('click', 'case-circles', clickHandlerRef.current);
    clickHandlerRef.current = (e) => {
      ...
      const html = `<div style="font-family:'Share Tech Mono',monospace;background:#060d15;border:1px solid #2a4a6a;padding:14px;min-width:180px;">
      ...
      popupRef.current = new ml.Popup({ closeButton: false, maxWidth: '260px' }).setLngLat(e.lngLat).setHTML(html).addTo(map);
    };
    ...
    mapClickHandlerRef.current = (e) => {
      ...
      const html  = `<div style="font-family:'Share Tech Mono',monospace;background:#060d15;border:1px solid #2a4a6a;padding:14px;min-width:200px;">
        <div style="color:#e8f4ff;font-size:12px;font-family:'Rajdhani',sans-serif;font-weight:600;margin-bottom:8px;letter-spacing:0.05em;">${(p.name || '').toUpperCase()} — ΖΩΝΗ ${label} ΚΙΝΔΥΝΟΥ</div>
      ...
      popupRef.current = new ml.Popup({ closeButton: false, maxWidth: '290px' }).setLngLat(e.lngLat).setHTML(html).addTo(map);
    };
    ...
    function animateHeat(ts) {
      ...
      if (map.getLayer('infection-heat')) map.setPaintProperty('infection-heat', 'heatmap-intensity', intensity);
      animRef.current = requestAnimationFrame(animateHeat);
    }
    animRef.current = requestAnimationFrame(animateHeat);
  }, [data, mapLoaded]);

  const topH = isMobile ? 44 : 48;
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', background: BG, overflow: 'hidden' }}>
      ...
      <TopBar news={data.news} datetime={datetime} isMobile={isMobile} />
      ...
      {/* Panels */}
      <LeftPanel
        news={data.news} onSelect={setSelectedNews}
        isMobile={isMobile} isOpen={openPanel === 'news'} onClose={closePanel}
      />
      <GlobalPanel
        confirmed={data.confirmed} suspected={data.suspected} news={data.news}
        isMobile={isMobile} isOpen={openPanel === 'stats'} onClose={closePanel}
      />
      <BottomHUD
        confirmed={data.confirmed} suspected={data.suspected}
        news={data.news} globalStats={data.globalStats}
        isMobile={isMobile} isOpen={openPanel === 'hud'} onClose={closePanel}
      />

      {/* Mobile toggle tabs (shown when panel is closed) */}
      {isMobile && openPanel !== 'news' && (
        <MobileTab side="left"   label="ΝΕΩΣ"  color={CYAN}   onClick={() => togglePanel('news')} />
      )}
      {isMobile && openPanel !== 'stats' && (
        <MobileTab side="right"  label="ΣΤΑΤ"  color={ORANGE} onClick={() => togglePanel('stats')} />
      )}
      {isMobile && openPanel !== 'hud' && (
        <MobileTab side="bottom" label="HUD"   color={DIM}    onClick={() => togglePanel('hud')} />
      )}
